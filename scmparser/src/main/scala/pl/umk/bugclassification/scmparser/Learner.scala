...
class Learner(private val projectInvokers: Map[String, ProjectInvoker]) {
  ...
  def trainAll = {
    projectInvokers.values.par.foreach(projectInvoker => projectInvoker.self ! Learn("master"))
  }

  def trainOnProject(project: String) = {
    projectInvokers.get(project).foreach(projectInvoker => projectInvoker.self ! Learn("master"))
  }

  def trainOnProject(project: String, branch: String) = {
    projectInvokers.get(project).foreach(projectInvoker => projectInvoker.self ! Learn(branch))
  }
}